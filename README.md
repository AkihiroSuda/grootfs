...
| clean.cache\_size | Disk usage of the store directory at which cleanup should trigger |
...
The `clean` command has a `cache-size` parameter, which when set, will try to 
keep the cache smaller than `cache-size` by cleaning unused layers.
The `cache-size` parameter defaults to 0, which always removes all unused layers.
