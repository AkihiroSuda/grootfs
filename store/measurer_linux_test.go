...
var _ = Describe("Measurer", func() {
	...
	FDescribe("MeasureCache", func() {
		BeforeEach(func() {
			volumeDriver.VolumesReturns([]string{"sha256:fake"})
		...
		It("measures the size of the cache (everything in the store, except images)", func() {
			...
			cacheSize, err := storeMeasurer.MeasureCache(logger)
			Expect(err).NotTo(HaveOccurred())

			Expect(volumeDriver.VolumesCallCount()).To(Equal(1))
			Expect(volumeDriver.VolumeSizeCallCount()).To(Equal(1))
			Expect(volumeDriver.VolumeSizeArgsForCall(0)).To(Equal("sha256:fake"))

			Expect(cacheSize).To(BeNumerically("==", 3*2048*1024))
		})
		...
		Context("when the store does not exist", func() {
			...
			It("returns a useful error", func() {
				_, err := storeMeasurer.MeasureCache(logger)
				Expect(err).To(MatchError(ContainSubstring("Invalid path /path/to/non/existent/store")))
			})
		})
	})
